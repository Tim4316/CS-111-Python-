...
class Board:
    ...
    def can_add_to(self, col):
        """ returns True if it is valid to place a checker in the column col 
            on the calling Board object.
        """
        
        if col < self.width and col > -1 and self.slots[0][col] == ' ':
            return True
        else:
            return False
    ...
    def is_up_diagonal_win(self, checker):
        ...
        for row in range(self.height - 3):
            n -= 1
            for col in range(self.width - 3):
                if self.slots[n][col] == checker and \
                    self.slots[n - 1][col + 1] == checker and \
                        self.slots[n - 2][col + 2] == checker and \
                            self.slots[n - 3][col + 3] == checker:
                                return True
        return False
