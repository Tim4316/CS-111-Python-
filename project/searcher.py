...
def h1(state):
    """ returns an estimate of how many additional moves are needed to get from 
        state to the goal state. """
    return state.board.num_misplaced()

def h2(state):
    """ should never overestimate the number of remaining moves """
    return state.board.misplaced_length()
...
class AStarSearcher(GreedySearcher):
    """ searcher objects that perform A* search """
    def priority(self, state):
        """ selects the next state based on those priorities. """
